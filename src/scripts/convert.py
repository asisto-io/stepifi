...
def merge_planar_faces(shape, tolerance=0.01):
    try:
        # removeSplitter() requires a tolerance argument in most FreeCAD versions
        merged = shape.removeSplitter(tolerance)
        return merged, True
    except Exception as e:
        # If removeSplitter fails, try without argument for backwards compatibility
        try:
...
def convert(input_path, output_path, tolerance=0.01, repair=True, info_only=False):
    ...
    final, merged_ok = merge_planar_faces(final, tolerance)
